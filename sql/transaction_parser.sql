...
BEGIN
     ...
     items := ITEMSARRAY('''$sequenceno$''', '$txntime$', '$txntype$', '$txnauth$', '$txnamount$', '$txnresult$', '''$txncurrency$''',
     ...
     cardno, cardtype, cardgroup, cardproduct, cardexpirydate, hosttxnstatus, devicetxnstatus, senddevicenumber, servicecode)
     VALUES (:1, ''$sequenceno$'', $txntime$, $txntype$, $txnauth$, $txnamount$, $txnresult$, ''$txncurrency$'',
     ''$cardno$'', $cardtype$, ''$cardgroup$'', ''$cardproduct$'', $cardexpirydate$, $hosttxnstatus$, $devicetxnstatus$, ''$senddevicenumber$'', ''$servicecode$'')';
